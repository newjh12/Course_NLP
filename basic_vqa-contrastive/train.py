...
from utils.contrastive import mask_correlated_samples, NT_Xent
        ...
        for phase in ['train', 'valid']:
            ...
            for batch_idx, batch_sample in enumerate(data_loader[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    output, img_feature, qst_feature, qst_ans_feature = model(image, question, label_onehot)      # [batch_size, ans_vocab_size=1000]
                    _, pred_exp1 = torch.max(output, 1)  # [batch_size]
                    _, pred_exp2 = torch.max(output, 1)  # [batch_size]
                    loss = criterion(output, label)
                    contrastive_criterion = SimCLR_Loss(img_feature.shape[0], temperature=0.5)
                    contrative_loss = contrastive_criterion(img_feature, qst_ans_feature)
                    total_loss = loss + contrative_loss * 0.6
                ...
                if batch_idx % 100 == 0:
                    print('| {} SET | Epoch [{:02d}/{:02d}], Step [{:04d}/{:04d}], Loss: {:.4f}, Ctr_Loss: {:.4f}'
                          .format(phase.upper(), epoch+1, args.num_epochs, batch_idx, int(batch_step_size), loss.item(), contrative_loss.item()))

            # Print the average loss and accuracy in an epoch.
            epoch_loss = running_loss / batch_step_size
            epoch_ctrloss = ctr_loss / batch_step_size
            ...
            print('| {} SET | Epoch [{:02d}/{:02d}], Loss: {:.4f}, CTR_Loss: {:.4f}, Acc(Exp1): {:.4f}, Acc(Exp2): {:.4f} \n'
                  .format(phase.upper(), epoch+1, args.num_epochs, epoch_loss, epoch_ctrloss, epoch_acc_exp1, epoch_acc_exp2))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='./result_qa/ctr0.6/logs',
                        help='directory for logs.')

    parser.add_argument('--model_dir', type=str, default='./result_qa/ctr0.6/models',
